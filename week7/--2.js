...
// HINT: Use firebaseLink.on("value", function(snapshot)) {}
	var clickData = new Firebase("https://arnoldleitestrcb.firebaseio.com/");

	clickData.on('value', function(data){
		console.log(data.val());
	
		data.val().clickCount = initialValue;
		clickCounter = data.val().clickCount;
		$('#clickValue').html(clickCounter);

	}, function(errorObject){
		conosle.log('something has gone wrong')
	})
